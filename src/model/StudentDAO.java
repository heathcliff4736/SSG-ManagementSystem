...
import src.vo.PersonVO;
import src.vo.StudentVO;
import util.DBUtil;
...
public class StudentDAO implements Student {
    ...
    public void totalSearch(int sortNum) {
        ...
        switch (sortNum) {
            case 1 -> selectsql = "SELECT * FROM STUDENT ORDER BY name";
            case 2 -> selectsql = "SELECT * FROM STUDENT ORDER BY sno";
            case 3 -> selectsql = "SELECT * FROM STUDENT ORDER BY korean";
        }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                studentVO.setMath(rs.getInt("math"));
                studentVO.setScience(rs.getInt("science"));
                String str = "\t%-10s%-10s%-10d%-10d%-10d%-10d%-10d%-10.1f%-5s\n";
                System.out.printf(
                        String.format(str,
                                studentVO.getSno(),
                                studentVO.getName(),
    ...
    public void search(String searchNum) {
        ...
        try {
            ...
            while (rs.next()) {
                StudentVO studentVO = new StudentVO();
                studentVO.setSno(rs.getString(1));
                studentVO.setName(rs.getString(2));
                studentVO.setKorean(rs.getInt(3));
                studentVO.setEnglish(rs.getInt(4));
                studentVO.setMath(rs.getInt(5));
                studentVO.setScience(rs.getInt(6));
                String str = "\t%-10s%-10s%-10d%-10d%-10d%-10d%-10d%-10.1f%-5s\n";
                System.out.printf(
                        String.format(str,
                                studentVO.getSno(),
